...
/*   Updated: 2025/12/25 14:20:04 by soamraou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || fd >= 1024 || BUFFER_SIZE < 1)
		return (NULL);
	if (stach[fd] == NULL)
		stach[fd] = ft_calloc(1, 1);
	while (stach[fd] != NULL && !ft_strchr(stach[fd], '\n'))
	{
		old_len = ft_strlen(stach[fd]);
		stach[fd] = fill_stach(stach[fd], fd);
		if (stach[fd] == NULL || ft_strlen(stach[fd]) == old_len)
			break ;
	}
	if (stach[fd] == NULL || stach[fd][0] == '\0')
	{
		free(stach[fd]);
		stach[fd] = NULL;
		return (NULL);
	}
	line = filter_getline(stach[fd]);
	stach[fd] = filter_stach(stach[fd], ft_strlen(line));
	return (line);
}
